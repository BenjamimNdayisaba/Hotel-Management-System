...
connection = mysql.connector.connect(
    ...
    database=config.get("DB_NAME", "hms"),
    port=config.get("DB_PORT", 3306),
    autocommit=config.get("DB_AUTOCOMMIT", True),
)
...
def get_guests():
    try:
        cursor.execute("SELECT id, name, address, email_id, phone, created_at FROM guests")
        return cursor.fetchall()
    except Exception as e:
        # Optionally log the error
        return []  # Return empty list instead of False or Nones
    return cursor.fetchall()
...
def get_reservations():
    ...
    cursor.execute(cmd)
    return cursor.fetchall()  # Always returns a list, even if empty
...
def meals():
    try:
        cursor.execute("SELECT COUNT(*) FROM reservations WHERE meal=1")
        result = cursor.fetchone()
        meals = result[0] if result and result[0] is not None else 0
    except Exception:
        meals = 0
